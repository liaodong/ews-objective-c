...
+ (BOOL) isValid:(MPSEWSContactItemType*) val forVersion:(MPSEWSExchangeVersion) ver
{   (void) val;
    ...
    if ([val fileAsMapping] && ![MPSEWSFileAsMappingType isValid:[val fileAsMapping] forVersion:ver]) return FALSE;
    if ([val displayName] && ![MPSEWSStringTypeHandler isValid:[val displayName] forVersion:ver]) return FALSE;
    if ([val givenName] && ![MPSEWSStringTypeHandler isValid:[val givenName] forVersion:ver]) return FALSE;
    ...
    if ([val completeName] && ![MPSEWSCompleteNameType isValid:[val completeName] forVersion:ver]) return FALSE;
    if ([val companyName] && ![MPSEWSStringTypeHandler isValid:[val companyName] forVersion:ver]) return FALSE;
    if ([val emailAddresses] && ![MPSEWSEmailAddressDictionaryType isValid:[val emailAddresses] forVersion:ver]) return FALSE;
    if ([val physicalAddresses] && ![MPSEWSPhysicalAddressDictionaryType isValid:[val physicalAddresses] forVersion:ver]) return FALSE;
    if ([val phoneNumbers] && ![MPSEWSPhoneNumberDictionaryType isValid:[val phoneNumbers] forVersion:ver]) return FALSE;
    if ([val assistantName] && ![MPSEWSStringTypeHandler isValid:[val assistantName] forVersion:ver]) return FALSE;
    if ([val birthday] && ![MPSEWSDateTimeTypeHandler isValid:[val birthday] forVersion:ver]) return FALSE;
    if ([val businessHomePage] && ![MPSEWSAnyUriTypeHandler isValid:[val businessHomePage] forVersion:ver]) return FALSE;
    if ([val children] && ![MPSEWSArrayOfStringsType isValid:[val children] forVersion:ver]) return FALSE;
    if ([val companies] && ![MPSEWSArrayOfStringsType isValid:[val companies] forVersion:ver]) return FALSE;
    if ([val contactSource] && ![MPSEWSContactSourceType isValid:[val contactSource] forVersion:ver]) return FALSE;
    if ([val department] && ![MPSEWSStringTypeHandler isValid:[val department] forVersion:ver]) return FALSE;
    if ([val generation] && ![MPSEWSStringTypeHandler isValid:[val generation] forVersion:ver]) return FALSE;
    if ([val imAddresses] && ![MPSEWSImAddressDictionaryType isValid:[val imAddresses] forVersion:ver]) return FALSE;
    if ([val jobTitle] && ![MPSEWSStringTypeHandler isValid:[val jobTitle] forVersion:ver]) return FALSE;
    if ([val manager] && ![MPSEWSStringTypeHandler isValid:[val manager] forVersion:ver]) return FALSE;
    if ([val mileage] && ![MPSEWSStringTypeHandler isValid:[val mileage] forVersion:ver]) return FALSE;
    if ([val officeLocation] && ![MPSEWSStringTypeHandler isValid:[val officeLocation] forVersion:ver]) return FALSE;
    if ([val postalAddressIndex] && ![MPSEWSPhysicalAddressIndexType isValid:[val postalAddressIndex] forVersion:ver]) return FALSE;
    if ([val profession] && ![MPSEWSStringTypeHandler isValid:[val profession] forVersion:ver]) return FALSE;
    if ([val spouseName] && ![MPSEWSStringTypeHandler isValid:[val spouseName] forVersion:ver]) return FALSE;
    if ([val surname] && ![MPSEWSStringTypeHandler isValid:[val surname] forVersion:ver]) return FALSE;
    if ([val weddingAnniversary] && ![MPSEWSDateTimeTypeHandler isValid:[val weddingAnniversary] forVersion:ver]) return FALSE;
    if ([val hasPicture] && ![MPSEWSBooleanTypeHandler isValid:[val hasPicture] forVersion:ver]) return FALSE;
    if ([val phoneticFullName] && ![MPSEWSStringTypeHandler isValid:[val phoneticFullName] forVersion:ver]) return FALSE;
    if ([val phoneticFirstName] && ![MPSEWSStringTypeHandler isValid:[val phoneticFirstName] forVersion:ver]) return FALSE;
    if ([val phoneticLastName] && ![MPSEWSStringTypeHandler isValid:[val phoneticLastName] forVersion:ver]) return FALSE;
    if ([val alias] && ![MPSEWSStringTypeHandler isValid:[val alias] forVersion:ver]) return FALSE;
    if ([val notes] && ![MPSEWSStringTypeHandler isValid:[val notes] forVersion:ver]) return FALSE;
    if ([val photo] && ![MPSEWSBase64BinaryTypeHandler isValid:[val photo] forVersion:ver]) return FALSE;
    if ([val userSMIMECertificate] && ![MPSEWSArrayOfBinaryType isValid:[val userSMIMECertificate] forVersion:ver]) return FALSE;
    if ([val mSExchangeCertificate] && ![MPSEWSArrayOfBinaryType isValid:[val mSExchangeCertificate] forVersion:ver]) return FALSE;
    if ([val directoryId] && ![MPSEWSStringTypeHandler isValid:[val directoryId] forVersion:ver]) return FALSE;
    if ([val managerMailbox] && ![MPSEWSSingleRecipientType isValid:[val managerMailbox] forVersion:ver]) return FALSE;
    if ([val directReports] && ![MPSEWSArrayOfRecipientsType isValid:[val directReports] forVersion:ver]) return FALSE;
    return TRUE;
}
