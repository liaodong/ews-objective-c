...
#import "MPSEWSAcceptSharingInvitationType.h"
...
@implementation MPSEWSAcceptSharingInvitationType 

+ (void) initialize
{
    MPSEWSObjectTypeHandler* handler = [[MPSEWSObjectTypeHandler alloc] initWithClass:[MPSEWSAcceptSharingInvitationType class]];
...
{   (void) val;
    if (![MPSEWSReferenceItemResponseType isValid:val forVersion:ver]) return FALSE;
    return TRUE;
}
...
- (Class) handlerClass
{
    return [MPSEWSAcceptSharingInvitationType class];
}

- (NSString*) description
{
    return [NSString stringWithFormat:@"AcceptSharingInvitationType: super=%@", [super description]];
}
