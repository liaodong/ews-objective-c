...
{   (void) val;
    ...
    if ([val actualWork] && ![MPSEWSIntegerTypeHandler isValid:[val actualWork] forVersion:ver]) return FALSE;
    if ([val assignedTime] && ![MPSEWSDateTimeTypeHandler isValid:[val assignedTime] forVersion:ver]) return FALSE;
    if ([val billingInformation] && ![MPSEWSStringTypeHandler isValid:[val billingInformation] forVersion:ver]) return FALSE;
    if ([val changeCount] && ![MPSEWSIntegerTypeHandler isValid:[val changeCount] forVersion:ver]) return FALSE;
    if ([val companies] && ![MPSEWSArrayOfStringsType isValid:[val companies] forVersion:ver]) return FALSE;
    if ([val completeDate] && ![MPSEWSDateTimeTypeHandler isValid:[val completeDate] forVersion:ver]) return FALSE;
    if ([val contacts] && ![MPSEWSArrayOfStringsType isValid:[val contacts] forVersion:ver]) return FALSE;
    if ([val delegationState] && ![MPSEWSTaskDelegateStateType isValid:[val delegationState] forVersion:ver]) return FALSE;
    if ([val delegator] && ![MPSEWSStringTypeHandler isValid:[val delegator] forVersion:ver]) return FALSE;
    if ([val dueDate] && ![MPSEWSDateTimeTypeHandler isValid:[val dueDate] forVersion:ver]) return FALSE;
    if ([val isAssignmentEditable] && ![MPSEWSIntegerTypeHandler isValid:[val isAssignmentEditable] forVersion:ver]) return FALSE;
    if ([val isComplete] && ![MPSEWSBooleanTypeHandler isValid:[val isComplete] forVersion:ver]) return FALSE;
    if ([val isRecurring] && ![MPSEWSBooleanTypeHandler isValid:[val isRecurring] forVersion:ver]) return FALSE;
    if ([val isTeamTask] && ![MPSEWSBooleanTypeHandler isValid:[val isTeamTask] forVersion:ver]) return FALSE;
    if ([val mileage] && ![MPSEWSStringTypeHandler isValid:[val mileage] forVersion:ver]) return FALSE;
    if ([val owner] && ![MPSEWSStringTypeHandler isValid:[val owner] forVersion:ver]) return FALSE;
    if ([val percentComplete] && ![MPSEWSDoubleTypeHandler isValid:[val percentComplete] forVersion:ver]) return FALSE;
    if ([val recurrence] && ![MPSEWSTaskRecurrenceType isValid:[val recurrence] forVersion:ver]) return FALSE;
    if ([val startDate] && ![MPSEWSDateTimeTypeHandler isValid:[val startDate] forVersion:ver]) return FALSE;
    if ([val status] && ![MPSEWSTaskStatusType isValid:[val status] forVersion:ver]) return FALSE;
    if ([val statusDescription] && ![MPSEWSStringTypeHandler isValid:[val statusDescription] forVersion:ver]) return FALSE;
    if ([val totalWork] && ![MPSEWSIntegerTypeHandler isValid:[val totalWork] forVersion:ver]) return FALSE;
    return TRUE;
}
