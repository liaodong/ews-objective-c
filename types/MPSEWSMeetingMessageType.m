...
@implementation MPSEWSMeetingMessageType 

+ (void) initialize
{
    MPSEWSObjectTypeHandler* handler = [[MPSEWSObjectTypeHandler alloc] initWithClass:[MPSEWSMeetingMessageType class]];
...
- (Class) handlerClass
{
    return [MPSEWSMeetingMessageType class];
}

- (NSString*) description
{
    return [NSString stringWithFormat:@"MeetingMessageType: AssociatedCalendarItemId=%@ IsDelegated=%@ IsOutOfDate=%@ HasBeenProcessed=%@ ResponseType=%@ UID=%@ RecurrenceId=%@ DateTimeStamp=%@ super=%@", _associatedCalendarItemId, _isDelegated, _isOutOfDate, _hasBeenProcessed, _responseType, _uID, _recurrenceId, _dateTimeStamp, [super description]];
}
