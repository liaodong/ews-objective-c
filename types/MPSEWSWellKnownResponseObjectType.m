...
@implementation MPSEWSWellKnownResponseObjectType 

+ (void) initialize
{
    MPSEWSObjectTypeHandler* handler = [[MPSEWSObjectTypeHandler alloc] initWithClass:[MPSEWSWellKnownResponseObjectType class]];
...
{   (void) val;
    if (![MPSEWSResponseObjectType isValid:val forVersion:ver]) return FALSE;
    return TRUE;
}
...
- (Class) handlerClass
{
    return [MPSEWSWellKnownResponseObjectType class];
}

- (NSString*) description
{
    return [NSString stringWithFormat:@"WellKnownResponseObjectType: super=%@", [super description]];
}
